import torch
from lib.qlinear4bit.nn import LinearQuant4bitDUASQ, Linear4bitDUSQ, SymQuantizer, AsymQuantizer, LinearQuant8bitASQ
import time
import rich
...
def show_tables(title, data_8b, data_4b, data_baseline):
    ...
    table.add_row("LinearFP16", *[data_baseline[key] for key in data_baseline.keys()])
    table.add_row("LinearInt8(with quantizer)", *[data_8b[key] for key in data_8b.keys()])
    table.add_row("LinearInt4(with quantizer)", *[data_4b[key] for key in data_4b.keys()])
    table.add_section()
    table.add_row("[green]Speedup\[FP16->INT8][/green]", *[f'{float(data_baseline[key].split()[0]) / float(data_8b[key].split()[0]):.3f}x' for key in data_8b.keys()])
    table.add_row("[green]Speedup\[FP16->INT4][/green]", *[f'{float(data_baseline[key].split()[0]) / float(data_4b[key].split()[0]):.3f}x' for key in data_4b.keys()])
    table.add_row("[green]Speedup\[INT8->INT4][/green]", *[f'{float(data_8b[key].split()[0]) / float(data_4b[key].split()[0]):.3f}x' for key in data_4b.keys()])
    rich.print(table)
...
def linear4bit_benchmark(args):
    ...
    for (feature_dim_in, feature_dim_out) in layer_size:
        for dtype in benchmark_dtypes:
            ...
            times_8bit = []
            for i in range(10):
                times_8bit.append(module_benchmark(int8asq_mod, x))
    ...
    show_tables("Linear Module Benchmark", data_8b, data_4b, data_baseline)
